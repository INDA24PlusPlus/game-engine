...
int main(int argc, char **argv) {
  ...
  state.multiplayer_client.connect_to_server(argv[1], argv[2]);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (input.is_key_just_pressed(GLFW_KEY_ESCAPE)) {
      ...
      glfwSetInputMode(window, GLFW_CURSOR,
                       state.mouse_locked ? GLFW_CURSOR_DISABLED
                                          : GLFW_CURSOR_NORMAL);
    }
    ...
    if (glm::length(direction) > 0) {
      if (input.is_key_pressed(GLFW_KEY_LEFT_SHIFT)) {
        ...
        player_rotation = glm::slerp(player_rotation, target_rotation,
        ...
        state.camera.m_pos = glm::mix(
            state.camera.m_pos, camera_target_position, state.delta_time * 5);
    ...
    state.multiplayer_client.send_position(player_position.x, player_position.z,
                                           0.0f);
