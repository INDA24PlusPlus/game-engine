...
struct SkinnedMeshPass {
    ...
    void init(Context* context, Buffer vertex_buffer);
    ...
    void create_descriptor_sets(Context* context);
    void create_descriptor_set_layout(Context* context);
    void create_uniform_buffers(Context* context);
    void create_pipeline(Context* context);
};


struct RenderState {
    SkinnedMeshPass skinned_mesh_pass;
...
VkPipeline build_pipeline(Context* context, VkPipelineLayout layout, Slice<VkVertexInputAttributeDescription> vertex_attributes, u32 vertex_stride, VkShaderModule vertex_shader, VkShaderModule fragment_shader);
static void allocate_descriptor_sets(Context* context, Slice<VkDescriptorSetLayout> layouts, Slice<VkDescriptorSet> sets);
...
struct StaticMeshPass {
    ...
    void render(VkCommandBuffer cmd, Slice<engine::asset::StaticMesh> meshes, u32 current_image_index) {
...
static void allocate_descriptor_sets(Context* context, Slice<VkDescriptorSetLayout> layouts, Slice<VkDescriptorSet> sets) {
    ...
    VK_CHECK(vkAllocateDescriptorSets(context->device, &alloc_info, sets.ptr));
}
...
static void create_descriptor_sets(Context* context) {
    ...
    allocate_descriptor_sets(context, { .ptr = layouts, .len = max_frames_in_flight }, { .ptr = context->state.descriptor_sets, .len = max_frames_in_flight });
...
static void update_uniform_buffers(Context* context, f32 curr_time, const glm::mat4& view_matrix) {
    ...
    // TODO: TEMP!
    memcpy(context->state.skinned_mesh_pass.ubo_matrices[context->swapchain.image_index], &mat_data, sizeof(UBOMatrices));
}
...
static void create_pipeline(Context* context) {
    ...
    VkPipelineLayout layout = context->state.pipeline_layout;
    Slice<VkVertexInputAttributeDescription> attribs = { .ptr = attribute_descriptions.data(), .len = attribute_descriptions.size() };

    VkShaderModule vertex_shader = load_shader_module(context, "shaders/vert.spv");
    VkShaderModule fragment_shader = load_shader_module(context, "shaders/frag.spv");
    context->state.pipeline = build_pipeline(context, layout, attribs, sizeof(engine::asset::Vertex), vertex_shader, fragment_shader);
}
...
template <typename T>
Slice<T> read_data_from_asset_file(Context* context, platform::OSHandle file, u32 count) {
    auto ret = arena_alloc<T>(&context->scratch, count);
    auto to_read = count * sizeof(T);
    auto size = platform::read_from_file(file, { .ptr = (u8*)ret.ptr, .len = to_read } );
    if (size != to_read) {
        platform::fatal("Failed to read data from asset file!");
    }
...
static void load_mesh_data(Context* context) {
    ...
    // Allocate enough space for the mesh transforms that will be populated during runtime.
    context->state.mesh_transforms = arena_alloc<glm::mat4>(&context->scratch, header.num_static_meshes + header.num_skinned_meshes);

    // Load CPU asset data.
    context->state.static_meshes = read_data_from_asset_file<engine::asset::StaticMesh>(context, file, header.num_static_meshes);
    context->state.skinned_meshes = read_data_from_asset_file<engine::asset::SkinnedMesh>(context, file, header.num_skinned_meshes);
    context->state.node_hierarchy = read_data_from_asset_file<engine::asset::Node>(context, file, header.num_nodes);
    context->state.bone_data = read_data_from_asset_file<engine::asset::BoneData>(context, file, header.num_bones);
...
static void render_mesh(Context* context) {
    ...
    context->state.skinned_mesh_pass.render(cmd, context->state.skinned_meshes, current_image_index, context->state.display_bone);
...
void SkinnedMeshPass::init(Context* context, Buffer vertex_buffer) {
    // Descriptors
    this->create_uniform_buffers(context);
    this->create_descriptor_set_layout(context);
    this->create_descriptor_sets(context);
...
void SkinnedMeshPass::create_descriptor_sets(Context* context) {
    VkDescriptorSetLayout layouts[max_frames_in_flight];
    for (size_t i = 0; i < max_frames_in_flight; i++) {
        layouts[i] = descriptor_set_layout;
    }

    allocate_descriptor_sets(context, { .ptr = layouts, .len = max_frames_in_flight }, { .ptr = ubo_descriptors, .len = max_frames_in_flight });
...
void SkinnedMeshPass::create_uniform_buffers(Context* context) {
    VkDeviceSize buffer_size = sizeof(UBOMatrices);
    for (size_t i = 0; i < max_frames_in_flight; ++i) {
        ubo_buffers[i] = create_buffer(context,
                                       buffer_size,
                                       VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                       VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT | VMA_ALLOCATION_CREATE_MAPPED_BIT);
...
void SkinnedMeshPass::create_pipeline(Context* context) {
    ...
    VkShaderModule vertex_shader = load_shader_module(context, "shaders/skinning_vert.spv");
    VkShaderModule fragment_shader = load_shader_module(context, "shaders/skinning_frag.spv");
    pipeline = build_pipeline(context, pipeline_layout, attribs, sizeof(engine::asset::SkinnedVertex), vertex_shader, fragment_shader);
}
...
void create_state_for_triangle(Context* context) {
    ...
    context->state.skinned_mesh_pass.init(context, context->state.skinned_vertex_buffer);
}
...
static void destroy_temp_state(Context* context) {
    ...
    // Skinned Pipeline
    context->state.skinned_mesh_pass.deinit(context);
