...
template <typename EnumType, std::size_t N> struct log_level_info {
...
constexpr auto log_level_list =
    log_level_info<LogLevels, static_cast<size_t>(LogLevels::COUNT)>{{{
...
private:
  static std::ostringstream &time_stream(std::ostringstream &stream,
                                         bool include_ms = true);
  static std::ostringstream &color_stream(std::ostringstream &stream, int R,
                                          int G, int B, bool bold = true);
};

template <LogLevels level, typename... Args>
void Logger::log_with_position(const std::string file_name_str,
  ...
  Logger::color_stream(stream, color.R, color.G, color.B)
      << " [" << file_name_str << ":" << line_number << "] "
      << log_level_list.to_string(level) << ": ";

  std::cout << stream.str() << std::format(fmt, std::forward<Args>(args)...)
            << ANSI_RESET << std::endl;
}
...
#define INFO(fmt, ...)                                          \
  LOGGER_LOG(LogLevels::INFO, fmt, ##__VA_ARGS__)
#define ERROR(fmt, ...)                                         \
  LOGGER_LOG(LogLevels::ERROR, fmt, ##__VA_ARGS__)
#define WARN(fmt, ...)                                          \
  LOGGER_LOG(LogLevels::WARN, fmt, ##__VA_ARGS__)
#define DEBUG(fmt, ...)                                         \
  LOGGER_LOG(LogLevels::DEBUG, fmt, ##__VA_ARGS__)
